...
- [ ] In the hosting panel, set `VSCODE_EXTENSION_ID=ddatunashvili.codecolab` in `.env` and restart
- [ ] Check `curl -s https://code-colab.renode.space/api/info` says `ddatunashvili.codecolab`
...
The "Open in VS Code" button builds `vscode://ddatunashvili.codecolab/join?...`.
...
```
VSCODE_EXTENSION_ID=ddatunashvili.codecolab
```
...
It must say `"extension_id":"ddatunashvili.codecolab"`. If it still says
...
```bash
npx @vscode/vsce unpublish ddatunashvili.codecolab
```
