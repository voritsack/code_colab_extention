...
- [ ] In the hosting panel, set `VSCODE_EXTENSION_ID=code-colab.codecolab` in `.env` and restart
- [ ] Check `curl -s https://code-colab.renode.space/api/info` says `code-colab.codecolab`
...
The "Open in VS Code" button builds `vscode://code-colab.codecolab/join?...`.
...
```
VSCODE_EXTENSION_ID=code-colab.codecolab
```
...
It must say `"extension_id":"code-colab.codecolab"`. If it still says
...
```bash
npx @vscode/vsce unpublish code-colab.codecolab
```
